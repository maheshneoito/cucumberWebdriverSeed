 ...
 */
export const resetBrowserUrl = async (): Promise<void> => {
    let baseURL: string = browser.config.baseUrl!;
    if (baseURL.includes("partstown.de")) {
        baseURL += "/en";
    }
    await browser.url(baseURL);
    await browser.pause(2000);
    await waitFor.UrlContains('partstown.');
}
